...
var usernameFuncs = {
  ...
  update: function(User, type, attributes, cb){
    User.update({username: type}, attributes).done(cb);
  }
};
...
var emailFuncs = {
  ...
  update: function(User, type, attributes, cb){
    User.update({email: type}, attributes).done(cb);
  }
};
    ...
    scope.findOrCreate(User, req.body, function(err, user){
      ...
      if (user) {
        if(bcrypt.compareSync(req.body.password, user.password)){
          ...
          // update the login
          scope.update(User, req.body[type], 
