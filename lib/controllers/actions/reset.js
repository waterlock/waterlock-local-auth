...
function sendEmail(req, res, sails, params){
  sails.models.auth.find({email: params.email}).limit(1).populate('user').exec(function(err, a){
    if(a instanceof Array) a = a[0];

    if(a){
      sails.models.resettoken.create({owner: a.id}).meta({fetch: true}).exec(function(err, t){
        if(err){
...
function issuePasswordReset(req, res, sails, params){
  sails.models.auth.find(req.session.resetToken.owner).limit(1).populate('resetToken').exec(function(err, auth){
    if(err){
      return res.serverError(err);
    }
    ...
    sails.models.auth.update({id:auth.id},{resetToken: null, password: params.password})
    .meta({fetch: true}).exec(function(err, auth){
      if(err){
        return res.serverError(err);
      }
...
function validateToken(req, res, sails, params){
  ...
  if(params.token){
    try{
      ...
      sails.models.auth.find(_token.iss).limit(1).populate('resetToken').exec(function(err, auth){
        if(auth instanceof Array) auth = auth[0];
